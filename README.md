...
OpenComposer は `v2.0.2`（commit `7af3d94b36043d8019b1639cd8e463957eb7a37e`）に固定し、スタックの Slurm を直接利用するよう構成します。OpenComposer には、`queues.conf` のパーティションとインスタンスタイプ（Slurm Constraint）を選択できる汎用 Slurm ジョブフォームも追加します。Constraint の候補には各パーティションの `instance_types[].name`、つまり生成される `slurm.conf` の `NodeName` における `Features` の最後の値を使用します。パーティションを変更すると、そのパーティションで利用できる Constraint だけが選択肢として有効になります。Shape で `VM Shape` を選ぶと、`#SBATCH --ntasks-per-core=1` と `#SBATCH --exclusive` をジョブスクリプトへ追加し、`BM.Optimized3.36/BM.HPC.E5.144` では追加しません。また、選択した Shape と MPI（`OpenMPI v4.x` または `Intel MPI(OneAPI)`）の組み合わせに対応する mpirun オプションを `export MPI_OPTIONS="..."` としてジョブスクリプトへ挿入します。フォームで生成したジョブスクリプトは投入前に編集できます。Ruby の依存 gem には Open OnDemand 4.0 が同梱する gem セットを利用します。
